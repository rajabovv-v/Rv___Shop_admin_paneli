import { Row, Form, Select, Col, Input, Button } from "antd";
...
function FormImages(props) {
  const { form } = props;
  const { response: categories, categoriesLoading } = useLoad({
    url: categoriesList,
  });
      ...
      <Col span={24}>
        ...
        >
          <Select
            ...
            onChange={(e) => form.setFieldValue("brand_id", e)}
      ...
      <Col span={24}>
        <Form.List name="images">
          {(fields, { add, remove }, { errors }) => (
            <>
              {fields.map((field) => (
                <Form.Item label="Image Url" key={field.key}>
                  <Form.Item
                    {...field}

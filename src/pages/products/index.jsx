...
function Products() {
  ...
  const handleRecommended = async (e, id) => {
    const { success } = await patchRequest.request({
      url: productsPatch(id),
      data: { isRecommended: e },
    });
    if (success) {
      reload();
    }
  ...
  const handleFinish = async (data) => {
    ...
    const { success } = isUpdate
      ? await patchRequest.request({ url: productsPatch(isUpdate), data })
  ...
  return (
    <div>
      ...
      >
        ...
        <Drawer
          title={`${isUpdate ? "Update" : "Add"} product`}
